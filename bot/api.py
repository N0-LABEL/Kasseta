...
def _is_ascii_latin(text: str) -> bool:
	...
	for ch in text or "":
		if 'A' <= ch <= 'Z' or 'a' <= ch <= 'z' or ch in allowed_extra:
			continue
		return False
	return True
...
class FootballAPI:
	...
	async def get_subject_suggestions(self, prefix: str, limit: int = 20) -> List[str]:
		if not (self.provider == "api_football" and self.api_key):
			return []
		# Only ASCII Latin allowed for suggestions
		if not _is_ascii_latin(prefix or ""):
			return []
	...
	async def verify_subject(self, name: str) -> Optional[Tuple[str, str]]:
		if not (self.provider == "api_football" and self.api_key):
			return None
		q = (name or "").strip()
		# Accept only ASCII latin; reject other scripts/Unicode
		if not _is_ascii_latin(q):
			return None
	...
	async def _ru_to_query(self, text: str, is_league: bool = False, fast: bool = False) -> str:
		# Kept for compatibility, but now enforcing ASCII latin externally
		q = (text or "").strip()
		return q

	async def _ru_to_en(self, text: str) -> str | None:
		# Not used under ASCII-only policy
		return None
